...
def multi_results(distance):
    ...
    if os.path.exists(str(distance) + '_2D'):
        pass
    else:
        os.makedirs(str(distance) + '_2D')
    save_path = str(distance) + '_2D'
    ...
    sns.kdeplot(r_data[:, 0], r_data[:, 1])
    fig.legend(["Marginal"])
    plt.xlim((-4, 4 + distance * 2))
    plt.ylim((-4,4))
    fig.show()
    fig.savefig(save_path+'/o_marginal.eps')
    ...
    for _ in range(40):
        for i in range(1000):
            ...
            if i % 10 == 0:
                ...
                G_loss = F.binary_cross_entropy(d_fake, torch.ones(256).cuda()) + F.cross_entropy(fake_cls,
                                                                                                  fake_label) - F.cross_entropy(
                    mi_c, fake_label)
    ...
    sns.kdeplot(g_data[:, 0], g_data[:, 1])
    fig.legend(["Marginal"])
    plt.xlim((-4, 4 + distance * 2))
    plt.ylim((-4, 4))
    fig.show()
    fig.savefig(save_path + '/twin_ac_marginal.eps')
    ...
    plt.ylim((-4, 4))
    fig.show()
    fig.savefig(save_path + '/ac_marginal.eps')
    ...
    plt.ylim((-4, 4))
    fig.show()
    fig.savefig(save_path + '/projection_marginal.eps')
